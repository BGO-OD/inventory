...
while ($row=pg_fetch_assoc($result)) {
	...
	echo "<td><input type=\"text\" name=\"comment\" size=\"50\" value=\"".$row['comment']."\"><input type=\"submit\" value=\"Submit\" ></td>";
	echo "</tr>\n";
 }
echo "</table>\n";


$result = pg_query($dbconn, "SELECT id,models.type,manufacturer,model,models.name,object_name,serial,objects.comment FROM locations INNER JOIN (objects INNER JOIN models USING (model)) USING (location) WHERE sublocations_parentlocation=$location;");
...
echo "<h2>Objects in this location</h2>";
echo "<table class=\"rundbtable\">\n";
echo "<tr class=\"rundbhead\">";
echo "<td>object</td>";
echo "<td>type</td>";
...
echo "<td>comment</td>";
echo "</tr>\n";
$result = pg_query($dbconn, "SELECT id,models.type,manufacturer,model,models.name,object_name,serial,objects.comment FROM locations INNER JOIN (objects INNER JOIN models USING (model)) USING (location) WHERE location=$location;");
while ($row=pg_fetch_assoc($result)) {
	echo "<tr class=\"rundbrun\">";
	echo "<td><a href=\"object.php?object='".$row['id']."'\">".$row['id']."</a></td>";
	echo "<td><a href=\"models.php?condition=type='".$row['type']."'\">".$row['type']."</a></td>";
...
echo "<td>id</td>";
echo "</tr>\n";
$result = pg_query($dbconn, "SELECT location,locations.type AS loctype ,locations.comment,models.type AS modtype ,manufacturer,name,id FROM locations LEFT OUTER JOIN (objects INNER JOIN models USING (model)) USING (location) WHERE parent_location=$location ORDER BY location;");
while ($row=pg_fetch_assoc($result)) {
	echo "<tr class=\"rundbrun\">";
