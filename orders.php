<?php
	...
	$result = pg_query($dbconn, "SELECT number, name, orderdate, invoicedate, netto, brutto, currency, amount, besteller, (select count(*) from ordercomments where ordercomments.number = orders.number) as comments,(SELECT name FROM users WHERE userid=signed_by) AS signature, (SELECT count(id) FROM objects WHERE order_number LIKE ('%05/' || substring(concat('0',number) from 3) || '%')) AS inventorycounts, (SELECT name FROM users WHERE userid=besteller) as bestellername FROM orders ORDER BY number DESC;");
